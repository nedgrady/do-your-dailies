...
func TestCreateChoreWithRealDBReturns201(t *testing.T) {
	t.Parallel()
	router := newTestRouter(NewGormStore(newCreateChoreTestDB(t)))
...
func TestCreateChoreWithRealDBPersistsChore(t *testing.T) {
	...
	router.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, "/api/chores/", createBody))

	listResponse := httptest.NewRecorder()
	router.ServeHTTP(listResponse, httptest.NewRequest(http.MethodGet, "/api/chores/", nil))
...
func TestCreateChoreWithTransactionDBReturns201(t *testing.T) {
	t.Parallel()
	router := newTestRouter(NewGormStore(newCreateChoreTestTransactionDB(t)))
